...
typedef
    void (*cascade_window_resize_callback)(struct cascade_window *, int, int);

void cascade_window_set_resize_callback(struct cascade_window **window,
        cascade_window_resize_callback callback);

/* 
 * Takes in a `struct cascade_window *self`, int key`, a `int action`,
 * and a `int mods`
 */
typedef 
    void (*cascade_window_key_callback)(struct cascade_window *, int, int, int);
